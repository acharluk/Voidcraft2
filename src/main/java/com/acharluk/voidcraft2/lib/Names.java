package com.acharluk.voidcraft2.lib;

/**
 * Created by ACharLuk on 23/06/2014.
 */
public class Names {

    public static class Blocks {

        public static final String voidOre = "void_ore";
        public static final String voidBlock = "void_block";
    }


    public static class Items {

        public static final String voidIngot = "void_ingot";
        public static final String voidShard = "void_shard";
        public static final String voidStick = "void_stick";


        public static final String voidSword = "void_sword";
        public static final String voidPickaxe = "void_pickaxe";
        public static final String voidShovel = "void_shovel";
        public static final String voidAxe = "void_axe";
        public static final String voidHoe = "void_hoe";
        public static final String voidSickle = "void_sickle";

    }

    public static class Fluids {

        public static final String voidFluid = "void_fluid";
        public static final String voidFluidBlock = "void_fluid_block";

    }

}
